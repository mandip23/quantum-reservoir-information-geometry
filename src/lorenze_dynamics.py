...
def runge_kutta4(n_steps=10000,dt=0.001,rho=28):
    state=np.array([1,1,1])
    series=[]
    for _ in range(10000):
      k1=lorenz_equations(state,rho=rho)
      k2=lorenz_equations(state+0.5*dt*k1,rho=rho)
      k3=lorenz_equations(state+0.5*dt*k2,rho=rho)
      k4=lorenz_equations(state+dt*k3,rho=rho)
