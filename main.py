...
from src.reservoir import create_pauli_operators
from src.embedding import reconstruct_phase_space, time_delay_embedding, normalize_embedding

from src.dynamicindicator import quantum_susceptibility_indicator, magnetization_variance , fidelity_autocorrelation_indicator, quantum_speed_indicator

# Import your simulation baseline functions
from src.lorenze_dynamics import runge_kutta4, regim_change
from src.reservoir import run_quantum_reservoir_simulation

def collect_ensemble_metrics(n_trials=5, window=200, is_control=False):
    
    # 1. Setup the physical system properties
    n_qubits = 4
    paulis = create_pauli_operators(n_qubits)
    
    # Storage structures for each metric timeline across trials
    trial_suscep = []
    trial_mag_var = []
    trial_speed_var = []
    trial_fid_corr = []
    
    # Get baseline trajectory dimensions to extract Takens embedding parameters
    base_raw = runge_kutta4(n_steps=10000, rho=28.0)
    ...
    for trial in range(n_trials):
        ...
        # 2. Generate raw dynamics data
        if is_control:
            # Control runs at constant chaotic attractor states (Rho=28)
            raw_series = runge_kutta4(n_steps=10000, rho=28.0)
            transition_trigger = 5000
        else:
            # Transition drops from 28 to 15 at step 5000
            raw_series, transition_trigger = regim_change(nstep=5000, rho1=28, rho2=15)
            
        # 3. Apply phase-space reconstruction pipeline
        X = time_delay_embedding(raw_series[:, 0], tau, dim)
        X_norm, _ = normalize_embedding(X)
        
        # Adjust transition index location based on Takens embedding offset truncation
        adjusted_trigger = transition_trigger - offset
        
        # 4. Spin up and run the open quantum reservoir simulation
        sim_data = run_quantum_reservoir_simulation(X_norm, n_qubits=n_qubits)
        rho_history = sim_data["rho_history"]
        
        # 5. Extract all custom structural early warning signals
        ...
        # Append timelines to tracking lists
...
def main():
    ...
    # Collect ensemble data matrices for both groups
    print("=========================================================")
    ...
    # -----------------------------------------------------------
    # GRAPH PLOTTING PIPELINE
    # -----------------------------------------------------------
   # -----------------------------------------------------------
    # # -----------------------------------------------------------
    # GRAPH PLOTTING PIPELINE (Robust Shape & Trigger Alignment)
    # -----------------------------------------------------------
    fig, axs = plt.subplots(4, 2, figsize=(16, 14), sharex=False, sharey='row') # Turned off sharex to handle shape steps cleanly
    ...
    titles = [
        ...
        "Quantum Speed Variance $\\text{Var}(V_{fro})$",
    ...
    for idx, key in enumerate(metrics_keys):
        ...
        # 1. Regenerate perfectly fitted coordinate spaces
        len_trans = len(m_trans)
        len_ctrl = len(m_ctrl)
        t_axis_trans = np.arange(len_trans)
        t_axis_ctrl = np.arange(len_ctrl)
        
        # 2. Dynamically rescale trigger positions to preserve mathematical mapping
        # This keeps the lines mathematically locked to the exact jump point
        adjusted_trans_trigger = int((trans["trigger"] / trans["length"]) * len_trans)
        adjusted_ctrl_trigger = int((ctrl["trigger"] / ctrl["length"]) * len_ctrl)
        ...
        axs[idx, 0].axvline(adjusted_trans_trigger, color='black', linestyle='--', lw=2, label='Regime Switch')
        axs[idx, 0].set_ylabel(titles[idx], fontweight='bold')
        axs[idx, 0].grid(True, linestyle=":", alpha=0.6)
        axs[idx, 0].set_xlim(0, len_trans) # Removes trailing whitespace gaps
        
        if idx == 0:
            axs[idx, 0].set_title("Transition Group: $\\rho_L = 28 \\rightarrow 15$", fontsize=12, fontweight='bold')
        ...
        axs[idx, 1].axvline(adjusted_ctrl_trigger, color='black', linestyle=':', lw=1.5, label='Reference Midpoint')
        axs[idx, 1].grid(True, linestyle=":", alpha=0.6)
        axs[idx, 1].set_xlim(0, len_ctrl) # Removes trailing whitespace gaps
        
        if idx == 0:
            axs[idx, 1].set_title("Control Group: Stationary Chaos ($\\rho_L = 28$)", fontsize=12, fontweight='bold')
            axs[idx, 1].legend(loc='upper left')

    # Set bottom labels cleanly
    ...
    plt.suptitle("Quantum Reservoir Early Warning Signal (EWS) Evaluation Matrix\n"
                 "Comparative Performance Verification of Custom Geometrical and Information Metrics", 
                 fontsize=14, fontweight='bold', y=0.97)
    
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    
    # Force save to disk before running the GUI viewer window
    output_png = "ews_custom_indicators_matrix.png"
    plt.savefig(output_png, dpi=300, bbox_inches='tight')
    
    print("\n=========================================================")
    print("[SUCCESS] Ensemble execution complete!")
    print(f"Matrix visualization exported to: {output_png}")
    print("=========================================================")
...
def main():
    ...
    for idx, key in enumerate(metrics_keys):
        ...
        len_trans, len_ctrl = len(m_trans), len(m_ctrl)
        t_axis_trans, t_axis_ctrl = np.arange(len_trans), np.arange(len_ctrl)
        
        # Rescale boundary triggers to perfectly match visual matrix resolution
        adj_trans_start = int((trans["start_trigger"] / trans["length"]) * len_trans)
        adj_trans_end = int((trans["end_trigger"] / trans["length"]) * len_trans)
        
        adj_ctrl_start = int((ctrl["start_trigger"] / ctrl["length"]) * len_ctrl)
        adj_ctrl_end = int((ctrl["end_trigger"] / ctrl["length"]) * len_ctrl)
        ...
        axs[idx, 0].set_ylabel(titles[idx], fontweight='bold')
        axs[idx, 0].grid(True, linestyle=":", alpha=0.6)
        axs[idx, 0].set_xlim(0, len_trans)
